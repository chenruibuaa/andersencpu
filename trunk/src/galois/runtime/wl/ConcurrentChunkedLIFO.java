...
class ConcurrentChunkedLIFO<T> implements Worklist<T> {
  ...
  public ConcurrentChunkedLIFO(Lambda0<Worklist<T>> maker, boolean needSize) {
    this(ChunkedLIFO.DEFAULT_CHUNK_SIZE, maker, needSize);
  }

  @SuppressWarnings("unchecked")
  public ConcurrentChunkedLIFO(int chunkSize, Lambda0<Worklist<T>> maker, boolean needSize) {
  ...
  public void add(T item, ThreadContext ctx) {
    ...
    current[getIndex(tid)].add(item, ctx);
  }
