...
class ConcurrentRandomOrder<T> implements Worklist<T> {
  ...
  public ConcurrentRandomOrder(Lambda0<Worklist<T>> maker, boolean needSize) {
    this(1024, maker, needSize);
  }

  @SuppressWarnings("unchecked")
  public ConcurrentRandomOrder(int startingSize, Lambda0<Worklist<T>> maker, boolean needSize) {
    rand = new Random();
  ...
  @Override
  public synchronized void add(T item, ThreadContext ctx) {
  ...
  @Override
  public synchronized T poll(ThreadContext ctx) {
    T retval = null;
    if (queueSize != 0) {
