/*
 ...
 File: ConcurrentLongSparseBitVector.java
 */
...
import util.ints.LongSet;
import util.ints.LongSetIterator;

import java.util.Iterator;
import java.util.concurrent.atomic.AtomicLongFieldUpdater;
import java.util.concurrent.atomic.AtomicReferenceFieldUpdater;

public final class ConcurrentLongSparseBitVector implements LongSet, Iterable<Long> {

    private static AtomicReferenceFieldUpdater<ConcurrentLongSparseBitVector, Element> firstUpdater
            = AtomicReferenceFieldUpdater.newUpdater(ConcurrentLongSparseBitVector.class, Element.class, "head");

    // 64 bits per element
    private static final long MASK = 64 - 1;
    private static final byte LOG_BITS_PER_ELEMENT = 6;

    private volatile Element head;
    private final ThreadLocal<Element> cursor;

    public ConcurrentLongSparseBitVector() {
        head = null;
    ...
    public boolean add(final long n) {
        long index = n >> LOG_BITS_PER_ELEMENT;
        long bits = 1L << (n & MASK);
        return add_(index, bits);
    }

    private boolean add_(long index, long bits) {
        ...
        for (; ; ) {
            if (prev == null) {
                ...
                if (currFirst == null || currFirst.index > index) {
                    ...
                    if (firstUpdater.compareAndSet(this, currFirst, newElement)) {
    ...
    public boolean unionTo(final ConcurrentLongSparseBitVector other) {
        boolean ret = false;
        Element otherCurr = other.head;
        while (otherCurr != null) {
            add_(otherCurr.index, otherCurr.bits);
    ...
    @NotThreadSafe
    public boolean serialAdd(final long n) {
        long index = n >> LOG_BITS_PER_ELEMENT;
        long bits = 1L << (n & MASK);
        return serialAdd_(index, bits);
    }

    @NotThreadSafe
    public boolean serialUnionTo(final ConcurrentLongSparseBitVector other) {
        boolean ret = false;
        Element otherCurr = other.head;
        while (otherCurr != null) {
    ...
    @NotThreadSafe
    private boolean serialAdd_(final long index, final long bits) {
        Element prev = find(getCursor(), head, index);
        if (prev == null) {
    ...
    public boolean addAll(LongSet longSet) {
        ...
        longSet.map(new LambdaVoid<Long>() {
            @Override
    ...
    public boolean equals(Object o) {
        ConcurrentLongSparseBitVector other = (ConcurrentLongSparseBitVector) o;
        Element elem = head;
