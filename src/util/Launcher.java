...
public class Launcher {
  ...
  /**
   * Adds statistics to current run. Only store statistics for the eventual
   * measured period between the last {@link #startTiming()} and first {@link #stopTiming()}.
   * 
  ...
  /**
   * Adds statistics to current run. Only store statistics for the eventual
   * measured period between the last {@link #startTiming()} and first
   * {@link #stopTiming()}.
   * 
   * @param stat
   *          the statistics to add
   * @param force
   *          add the statistics even if outside measured period
   */
  public void addStats(Statistics stat, boolean force) {
    if (stat != null && (stop == null || force))
      stats.add(stat);
  }
