...
class ParameterUnorderedExecutor<T> extends AbstractParameterExecutor<T, T> {
  ...
  private LambdaVoid<T> mappableBody;
  ...
  public ParameterStatistics call(LambdaVoid<T> body) throws ExecutionException {
    mappableBody = body;
    ParameterStatistics stats = super.call(null);
  ...
  protected Iteration newIteration() {
    return new Iteration(getThreadId());
  }
  ...
  protected void run(T cur, Iteration it) {
    ...
    try {
      ...
      commitPool.add(it);
      currStepRec.logExecuted(it, notUseful);
